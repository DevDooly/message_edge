...
안드로이드 OS 자체 보안 기능으로 인해 상단 바에 `Notification Edge이(가) 다른 앱 위에 표시됨` 알림이 계속 떠 있는 경우, 아래 방법으로 깔끔하게 끌 수 있습니다:

- **가장 빠른 방법**: 상단 바에서 해당 알림을 **길게 꾹 누름** ➔ **`[알림 끄기]`** (또는 스위치 OFF) ➔ **적용**
- *이 설정은 Android 시스템의 오버레이 안내 알림 카테고리만 끄는 것이므로, 알림 엣지 앱 동작에는 아무런 영향을 주지 않습니다.*
