...
갤럭시 스마트폰의 기본 **Edge 패널(시스템 핸들)**과 본 앱의 **알림 엣지 핸들**이 겹치거나 두 개의 핸들이 떠서 화면 터치가 불편할 경우, 아래 방법 중 편한 방식을 선택하여 설정하시면 간섭 없이 편리하게 사용하실 수 있습니다:

### 1️⃣ 추천 1: 반대편 배치 (좌/우 분리) ⭐ 가장 추천
...
### 2️⃣ 추천 2: 상/하 높이 분리 배치 (동일 방향 사용 시)
...
### 3️⃣ 추천 3: 핸들 바 화면 숨김 (제스처 전용 투명 모드)
- 본 앱 설정에서 **`핸들 바 화면 표시`** 스위치를 **OFF(끄기)**로 변경
- **효과**: 화면에 거슬리는 핸들 바 그래픽이 완전히 사라지며, 지정된 엣지 영역을 스와이프했을 때만 깔끔하게 알림 패널이 열립니다.
